...
The dashboard trains in short chunks, renders the current policy on an HTML Canvas, and lets you update live parameters such as learning rate, gamma, entropy coefficient, clip range, food reward, distance reward, food-time penalty, loop penalty, oscillation penalty, training chunk size, and preview length. Model-shape settings such as CNN/MLP, device, rollout steps, and batch size should be applied with **Reset With Config**.

Use **Download Model** to save a `.snakeai.zip` bundle containing the Stable-Baselines3 model plus dashboard metadata. Use **Import And Pause** to load that bundle later, restore the training step count/config/history, and continue by pressing **Start**.
